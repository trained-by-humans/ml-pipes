...
class Infer:
    def __init__(
        ...
        if not isinstance(model, torch.nn.Module):
            raise TypeError(f"Infer requires a torch.nn.Module, got {type(model)!r}")
        self.model = model
    ...
    def __call__(self, tensor_payload: TensorPayload) -> RuntimeOutputs:
        ...
        if self.model_dtype is not None and tensor_payload.dtype != self.model_dtype:
            raise ValueError(
                f"Infer expects model dtype {self.model_dtype}, got {tensor_payload.dtype}"
            )
        ...
        if self.output_layouts is None:
            ...
            if len(self.output_layouts) != len(output_tensors):
                raise ValueError(
                    f"Infer expected {len(self.output_layouts)} output layouts, got {len(output_tensors)} outputs"
                )
        ...
        return RuntimeOutputs(tensors=tensors, names=output_names)
    ...
    def _normalize_outputs(self, outputs: object) -> tuple[tuple[str, ...], tuple[torch.Tensor, ...]]:
        ...
        if isinstance(outputs, Sequence):
            ...
            if any(not isinstance(output, torch.Tensor) for output in output_tensors):
                raise TypeError(
                    "Infer supports only torch.Tensor outputs, sequences of torch.Tensor outputs, "
        ...
        if isinstance(outputs, Mapping):
            ...
            for name, output in outputs.items():
                if not isinstance(name, str):
                    raise TypeError(f"Infer mapping output names must be strings, got {type(name)!r}")
            ...
            raise TypeError("Infer mapping outputs must contain at least one torch.Tensor value")
        raise TypeError(
            "Infer supports only torch.Tensor outputs, sequences of torch.Tensor outputs, "
            "or mappings of named torch.Tensor outputs"
        )
